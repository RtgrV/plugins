...
+ (FLTDataMessage *)fromMap:(NSDictionary *)dict {
  ...
  NSAssert(pigeonResult.requestId != nil, @"");
...
- (NSDictionary *)toMap {
  return [NSDictionary dictionaryWithObjectsAndKeys:(self.textureId ? self.textureId : [NSNull null]), @"textureId", (self.requestId ? self.requestId : [NSNull null]), @"requestId", (self.headers ? self.headers : [NSNull null]), @"headers", (self.data ? self.data : [NSNull null]), @"data", nil];
}
@end
...
void FLTAVFoundationVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.setData"
        binaryMessenger:binaryMessenger
        codec:FLTAVFoundationVideoPlayerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(receiveData:error:)], @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(receiveData:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTDataMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api receiveData:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
